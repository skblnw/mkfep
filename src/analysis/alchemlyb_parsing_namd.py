...
Modified from alchemlyb.parsing.namd
"""
import logging
...
# from alchemlyb.parsing import _init_attrs  # error referring, manually set energy unit
...
# @_init_attrs  # error referring, manually set energy unit
def extract_u_nk(fep_files, T, all_lambdas=None, option='total'):
    ...
    for fep_file in sorted(fep_files, key=_filename_sort_key):
        ...
        with anyopen(fep_file, "r") as f:
            ...
            for line in f:
                ...
                # NOTE: modified here, to read elec,vdw altogether!
                if parsing:
                    if l[0] == "FepEnergy:":
                        if option == 'total':
                            win_de.append(round(float(l[6]), 8))
                        elif option == 'elec':
                            win_de.append(round(float(l[3])-float(l[2]), 8))
                        elif option == 'vdw':
                            win_de.append(round(float(l[5])-float(l[4]), 8))
